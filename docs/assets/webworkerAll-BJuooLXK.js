...
let __tla = Promise.all([
  ...
  class Pt extends lt {
    ...
    get font() {
      return T(R, "BitmapFont.font is deprecated, please use BitmapFont.fontFamily instead."), this.fontFamily;
    }
    get pageTextures() {
      return T(R, "BitmapFont.pageTextures is deprecated, please use BitmapFont.pages instead."), this.pages;
    }
    get size() {
      return T(R, "BitmapFont.size is deprecated, please use BitmapFont.fontMetrics.fontSize instead."), this.fontMetrics.fontSize;
    }
    get distanceFieldRange() {
      return T(R, "BitmapFont.distanceFieldRange is deprecated, please use BitmapFont.distanceField.range instead."), this.distanceField.range;
    }
    get distanceFieldType() {
      return T(R, "BitmapFont.distanceFieldType is deprecated, please use BitmapFont.distanceField.type instead."), this.distanceField.type;
    }
    destroy(e = false) {
      var _a;
  ...
  const De = class Oe extends Pt {
    ...
    ensureCharacters(e) {
      var _a, _b;
      const t = D.graphemeSegmenter(e).filter((x) => !this._currentChars.includes(x)).filter((x, p, g) => g.indexOf(x) === p);
      if (!t.length) return;
      this._currentChars = [
        ...this._currentChars,
        ...t
      ...
      for (let x = 0; x < t.length; x++) {
        const p = t[x], g = D.measureText(p, a, i, false);
        g.lineHeight = g.height;
        const b = g.width * c, C = Math.ceil((a.fontStyle === "italic" ? 2 : 1) * b), U = g.height * c, M = C + h * 2, G = U + h * 2;
        if (f = false, p !== `
` && p !== "\r" && p !== "	" && p !== " " && (f = true, d = Math.ceil(Math.max(G, d))), u + M > y && (l += d, d = G, u = 0, l + d > _)) {
          n.update();
          const A = this._nextPage();
          i = A.canvasAndContext.canvas, s = A.canvasAndContext.context, n = A.texture.source, u = 0, l = 0, d = 0;
        }
        const z = b / c - (((_a = a.dropShadow) == null ? void 0 : _a.distance) ?? 0) - (((_b = a._stroke) == null ? void 0 : _b.width) ?? 0);
        if (this.chars[p] = {
          id: p.codePointAt(0),
          ...
          this._drawGlyph(s, g, u + h, l + h, c, a);
          const A = n.width * c, le = n.height * c, ot = new ct(u / A * n.width, l / le * n.height, M / A * n.width, G / le * n.height);
          this.chars[p].texture = new w({
            source: n,
            frame: ot
          }), u += Math.ceil(M);
        }
    ...
    _nextPage() {
      ...
      const e = this.resolution, t = H.getOptimalCanvasAndContext(this._textureSize, this._textureSize, e);
    ...
    _setupContext(e, t, r) {
      t.fontSize = this.baseRenderedFontSize, e.scale(r, r), e.font = de(t), t.fontSize = this.baseMeasurementFontSize, e.textBaseline = t.textBaseline;
      const i = t._stroke, s = (i == null ? void 0 : i.width) ?? 0;
      if (i && (e.lineWidth = s, e.lineJoin = i.join, e.miterLimit = i.miterLimit, e.strokeStyle = he(i, e)), t._fill && (e.fillStyle = he(t._fill, e)), t.dropShadow) {
    ...
    _drawGlyph(e, t, r, i, s, n) {
      var _a;
      const a = t.text, u = t.fontProperties, d = (((_a = n._stroke) == null ? void 0 : _a.width) ?? 0) * s, c = r + d / 2, h = i - d / 2, f = u.descent * s, y = t.lineHeight * s;
      let _ = false;
      n.stroke && d && (_ = true, e.strokeText(a, c, h + y - f));
      const { shadowBlur: x, shadowOffsetX: p, shadowOffsetY: g } = e;
      n._fill && (_ && (e.shadowBlur = 0, e.shadowOffsetX = 0, e.shadowOffsetY = 0), e.fillText(a, c, h + y - f)), _ && (e.shadowBlur = x, e.shadowOffsetX = p, e.shadowOffsetY = g);
    }
    destroy() {
      super.destroy();
      for (let e = 0; e < this.pages.length; e++) {
        const { canvasAndContext: t, texture: r } = this.pages[e];
        H.returnCanvasAndContext(t), r.destroy(true);
      }
      this.pages = null;
    }
  };
  ...
  function Ie(o, e, t, r) {
    ...
    const u = {
      ...
      for (let b = 0; b < u.index; b++) {
        const C = p.positions[b];
        s.chars.push(p.chars[b]), s.charPositions.push(C + g);
      }
      s.width += p.width, a = false, u.width = 0, u.index = 0, u.chars.length = 0;
    }, _ = () => {
      let p = s.chars.length - 1;
      if (r) {
        let g = s.chars[p];
        for (; g === " "; ) s.width -= t.chars[g].xAdvance, g = s.chars[--p];
      }
      i.width = Math.max(i.width, s.width), s = {
        ...
        spaceWidth: 0,
        spacesIndex: []
      }, a = true, i.lines.push(s), i.height += h;
    }, x = (p) => p - d > c;
    for (let p = 0; p < o.length + 1; p++) {
      let g;
      const b = p === o.length;
      b || (g = o[p]);
      const C = t.chars[g] || t.chars[" "];
      if (/(?:\s)/.test(g) || g === "\r" || g === `
` || b) {
        if (!a && e.wordWrap && x(s.width + u.width) ? (_(), y(u), b || s.charPositions.push(0)) : (u.start = s.width, y(u), b || s.charPositions.push(0)), g === "\r" || g === `
`) _();
        else if (!b) {
          const z = C.xAdvance + (C.kerning[n] || 0) + d;
          s.width += z, s.spaceWidth = z, s.spacesIndex.push(s.charPositions.length), s.chars.push(g);
        }
      } else {
        const G = C.kerning[n] || 0, z = C.xAdvance + G + d;
        f && x(s.width + u.width + z) && (y(u), _()), u.positions[u.index++] = u.width + G, u.chars.push(g), u.width += z;
      }
      n = g;
    }
    return _(), e.align === "center" ? St(i) : e.align === "right" ? Ft(i) : e.align === "justify" && Bt(i), i;
  }
  function St(o) {
  ...
  function Bt(o) {
    ...
    for (let t = 0; t < o.lines.length; t++) {
      ...
      for (let d = 0; d < r.charPositions.length; d++) d === s && (s = r.spacesIndex[i++], n += l), r.charPositions[d] += n;
    }
  }
  function Rt(o) {
    if (o === "") return [];
    ...
    for (let t = 0, r = o.length; t < r; t++) {
      ...
      if (Array.isArray(i)) {
        ...
        for (let a = s, u = n; a <= u; a++) e.push(String.fromCharCode(a));
  ...
  class Mt {
    constructor() {
      this.ALPHA = [
        ...
        skipKerning: false,
        textureStyle: null
      }, this.measureCache = ht(1e3);
    }
    getFont(e, t) {
      var _a;
      let r = `${t.fontFamily}-bitmap`, i = true;
      if (t._fill.fill && !t._stroke ? (r += t._fill.fill.styleKey, i = false) : (t._stroke || t.dropShadow) && (r = `${t.styleKey}-bitmap`, i = false), !P.has(r)) {
        ...
        V++, V > 50 && Y("BitmapText", `You have dynamically created ${V} bitmap fonts, this can be inefficient. Try pre installing your font styles using \`BitmapFont.install({name:"style1", style})\``), a.once("destroy", () => {
          V--, P.remove(r);
        }), P.set(r, a);
      }
      const s = P.get(r);
    ...
    getLayout(e, t, r = true) {
      ...
      const n = D.graphemeSegmenter(e), a = Ie(n, t, i, r);
    ...
    install(...e) {
      ...
      }, T(R, "BitmapFontManager.install(name, style, options) is deprecated, use BitmapFontManager.install({name, style, ...options})"));
      ...
      const i = t.style, s = i instanceof W ? i : new W(i), n = t.dynamicFill ?? this._canUseTintForStyle(s), a = new fe({
        style: s,
        overrideFill: n,
...
`;
  ...
  function kt(o, e) {
    ...
    for (let r = 0; r < o.length; r++) {
      ...
      (s == null ? void 0 : s.isCachedAsTexture) ? e.matrix = ge.copyFrom(s.textureOffsetInverseTransform).append(i.worldTransform) : (s == null ? void 0 : s._parentCacheAsTextureRenderGroup) ? e.matrix = ge.copyFrom(s._parentCacheAsTextureRenderGroup.inverseWorldTransform).append(i.groupTransform) : e.matrix = i.worldTransform, e.addBounds(i.bounds);
    }
    return e.matrix = t, e;
  }
  const Dt = new re({
  ...
  class Ot {
    constructor() {
      this.skip = false, this.inputTexture = null, this.backTexture = null, this.filters = null, this.bounds = new Be(), this.container = null, this.blendRequired = false, this.outputRenderSurface = null, this.globalFrame = {
        x: 0,
        y: 0,
        width: 0,
        height: 0
  ...
  class Ee {
    constructor(e) {
      ...
      }), this._globalFilterBindGroup = new Fe({}), this.renderer = e;
    }
    ...
    push(e) {
      ...
      const u = i.bounds;
      if (this._calculateFilterArea(e, u), this._calculateFilterBounds(i, t.renderTarget.rootViewPort, a, n, 1), i.skip) return;
      const l = this._getPreviousFilterData(), d = this._findFilterResolution(n);
      let c = 0, h = 0;
      l && (c = l.bounds.minX, h = l.bounds.minY), this._calculateGlobalFrame(i, c, h, d, s.width, s.height), this._setupFilterTextures(i, u, t, l);
    }
    generateFilteredTexture({ texture: e, filters: t }) {
      ...
      const u = s;
      this._calculateGlobalFrame(r, 0, 0, u, i.width, i.height), r.outputRenderSurface = S.getOptimalTexture(a.width, a.height, r.resolution, r.antialias), r.backTexture = w.EMPTY, r.inputTexture = e, this.renderer.renderTarget.finishRenderPass(), this._applyFiltersToTexture(r, true);
      const h = r.outputRenderSurface;
      return h.source.alphaMode = "premultiplied-alpha", h;
    }
    pop() {
      const e = this.renderer, t = this._popFilterData();
      t.skip || (e.globalUniforms.pop(), e.renderTarget.finishRenderPass(), this._activeFilterData = t, this._applyFiltersToTexture(t, false), t.blendRequired && S.returnTexture(t.backTexture), S.returnTexture(t.inputTexture));
    }
    getBackTexture(e, t, r) {
      const i = e.colorTexture.source._resolution, s = S.getOptimalTexture(t.width, t.height, i, false);
    ...
    calculateSpriteMatrix(e, t) {
      const r = this._activeFilterData, i = e.set(r.inputTexture._source.width, 0, 0, r.inputTexture._source.height, r.bounds.minX, r.bounds.minY), s = t.worldTransform.copyTo(B.shared), n = t.renderGroup || t.parentRenderGroup;
    ...
    _getPassthroughFilter() {
      return this._passthroughFilter ?? (this._passthroughFilter = new At()), this._passthroughFilter;
    }
    ...
    _setupFilterTextures(e, t, r, i) {
      if (e.backTexture = w.EMPTY, e.inputTexture = S.getOptimalTexture(t.width, t.height, e.resolution, e.antialias), e.blendRequired) {
    ...
    _updateFilterUniforms(e, t, r, i, s, n, a, u) {
      ...
      y[0] = i * n, y[1] = s * n, y[2] = x.source.width * n, y[3] = x.source.height * n, t instanceof w && (t.source.resource = null);
      const p = this.renderer.renderTarget.getRenderTarget(t);
      this.renderer.renderTarget.bind(t, !!u), t instanceof w ? (_[0] = t.frame.width, _[1] = t.frame.height) : (_[0] = p.width, _[1] = p.height), _[2] = p.isRoot ? -1 : 1, this._filterGlobalUniforms.update();
    }
    ...
    _calculateFilterArea(e, t) {
      if (e.renderables ? kt(e.renderables, t) : e.filterEffect.filterArea ? (t.clear(), t.addRect(e.filterEffect.filterArea), t.applyMatrix(e.container.worldTransform)) : e.container.getFastGlobalBounds(true, t), e.container) {
    ...
    _applyFiltersToTexture(e, t) {
      ...
      if (this._globalFilterBindGroup.setResource(r.source.style, 2), this._globalFilterBindGroup.setResource(e.backTexture.source, 3), n === a) s[n].apply(this, r, e.outputRenderSurface, t);
        ...
        const l = S.getOptimalTexture(i.width, i.height, u.source._resolution, false);
        let d = l;
        for (let c = n; c < a; c++) {
          const h = s[c];
          if (!h.enabled) continue;
          h.apply(this, u, d, true);
          const f = u;
          u = d, d = f;
        }
        s[a].apply(this, u, e.outputRenderSurface, t), S.returnTexture(l);
      }
    }
    _calculateFilterBounds(e, t, r, i, s) {
      var _a;
      const n = this.renderer, a = e.bounds, u = e.filters;
      let l = 1 / 0, d = 0, c = true, h = false, f = false, y = true, _ = -1, x = -1;
      for (let p = 0; p < u.length; p++) {
        const g = u[p];
        if (!g.enabled) continue;
        if (_ === -1 && (_ = p), x = p, l = Math.min(l, g.resolution === "inherit" ? i : g.resolution), d += g.padding, g.antialias === "off" ? c = false : g.antialias === "inherit" && c && (c = r), g.clipToViewport || (y = false), !!!(g.compatibleRenderers & n.type)) {
          f = false;
          break;
        }
        if (g.blendRequired && !(((_a = n.backBuffer) == null ? void 0 : _a.useBackBuffer) ?? true)) {
          Y("Blend filter requires backBuffer on WebGL renderer to be enabled. Set `useBackBuffer: true` in the renderer options."), f = false;
          break;
        }
        f = true, h || (h = g.blendRequired);
      }
      if (!f) {
        e.skip = true;
        return;
      ...
      e.antialias = c, e.resolution = l, e.blendRequired = h, e.firstEnabledIndex = _, e.lastEnabledIndex = x;
    }
    _popFilterData() {
    ...
    _pushFilterData() {
      ...
      return e || (e = this._filterStack[this._filterStackIndex] = new Ot()), this._filterStackIndex++, e;
    }
  }
  Ee.extension = {
    type: [
  ...
  const Ve = class Le extends re {
    constructor(...e) {
      let t = e[0] ?? {};
      t instanceof Float32Array && (T(R, "use new MeshGeometry({ positions, uvs, indices }) instead"), t = {
        positions: t,
        uvs: e[1],
        indices: e[2]
      }), t = {
      ...
      const s = t.indices || new Uint32Array([
        ...
        2,
        3
      ]), n = t.shrinkBuffersToFit, a = new O({
        data: r,
  ...
  class He {
    constructor() {
      ...
      e.setAttribute("width", "10000"), e.setAttribute("height", "10000"), e.style.overflow = "hidden", t.appendChild(e), e.appendChild(r), e.appendChild(i), this.image = se.get().createImage();
    }
    destroy() {
  ...
  class Ye {
    ...
    execute(e) {
      ...
      n.uTransformMatrix = e.groupTransform, n.uRound = t._roundPixels | e._roundPixels, $(e.groupColorAlpha, n.uColor, 0), this._adaptor.execute(this, e);
    }
    _rebuild(e) {
  ...
  const Xe = class Ke extends oe {
    constructor(...e) {
      super({});
      let t = e[0] ?? {};
      typeof t == "number" && (T(R, "PlaneGeometry constructor changed please use { width, height, verticesX, verticesY } instead"), t = {
        width: t,
        height: e[1],
        verticesX: e[2],
        verticesY: e[3]
    ...
    build(e) {
      ...
      const d = n * a;
      for (let c = 0; c < d; c++) {
        const h = c % n, f = c / n | 0, y = f * this.verticesX + h, _ = f * this.verticesX + h + 1, x = (f + 1) * this.verticesX + h, p = (f + 1) * this.verticesX + h + 1;
        s.push(y, _, x, _, p, x);
      }
  ...
  class $e {
    constructor(e, t) {
      ...
      }), this.localUniformsBindGroup = new Fe({
    ...
    execute(e) {
      ...
      t.uniforms.uTransformMatrix = e.groupTransform, t.uniforms.uRound = this.renderer._roundPixels | e._roundPixels, t.update(), $(e.groupColorAlpha, t.uniforms.uColor, 0), this._adaptor.execute(this, e);
    }
    _getMeshData(e) {
      var t, r;
  ...
  function Ht(o) {
    return {
      dynamicUpdate: ve(o, true),
      staticUpdate: ve(o, false)
    };
  }
  function ve(o, e) {
    const t = [];
...
`);
  ...
  class Yt {
    constructor(e) {
      ...
      let i = 0, s = 0;
      for (const d in r) {
        const c = r[d], h = ee(c.format);
        c.dynamic ? s += h.stride : i += h.stride;
      }
      this._dynamicStride = s / 4, this._staticStride = i / 4, this.staticAttributeBuffer = new E(t * 4 * i), this.dynamicAttributeBuffer = new E(t * 4 * s), this.indexBuffer = be(t);
      const n = new re();
      let a = 0, u = 0;
    ...
    update(e, t) {
      e.length > this._size && (t = true, this._size = Math.max(e.length, this._size * 1.5 | 0), this.staticAttributeBuffer = new E(this._size * this._staticStride * 4 * 4), this.dynamicAttributeBuffer = new E(this._size * this._dynamicStride * 4 * 4), this.indexBuffer = be(this._size), this.geometry.indexBuffer.setDataWithSize(this.indexBuffer, this.indexBuffer.byteLength, true));
...
fn mainFragment(
  ...
  class je {
    constructor(e, t) {
      this.state = X.for2d(), this.localUniforms = new k({
        uTranslationMatrix: {
          value: new B(),
          type: "mat3x3<f32>"
        },
      ...
      }), this.renderer = e, this.adaptor = t, this.defaultShader = new jt(), this.state = X.for2d();
    }
    validateRenderable(e) {
      return false;
    }
    ...
    _initBuffer(e) {
      return e._gpuData[this.renderer.uid] = new Yt({
    ...
    execute(e) {
      ...
      const s = this.state;
      i.update(t, e._childrenDirty), e._childrenDirty = false, s.blendMode = ne(e.blendMode, e.texture._source);
      const n = this.localUniforms.uniforms, a = n.uTranslationMatrix;
      e.worldTransform.copyTo(a), a.prepend(r.globalUniforms.globalUniformData.projectionMatrix), n.uResolution = r.globalUniforms.globalUniformData.resolution, n.uRound = r._roundPixels | e._roundPixels, $(e.groupColorAlpha, n.uColor, 0), this.adaptor.execute(this, e);
    }
    destroy() {
  ...
  const Qe = class Je extends Et {
    ...
    updatePositions() {
      const e = this.positions, { width: t, height: r, _leftWidth: i, _rightWidth: s, _topHeight: n, _bottomHeight: a, _anchorX: u, _anchorY: l } = this, d = i + s, c = t > d ? 1 : t / d, h = n + a, f = r > h ? 1 : r / h, y = Math.min(c, f), _ = u * t, x = l * r;
      e[0] = e[8] = e[16] = e[24] = -_, e[2] = e[10] = e[18] = e[26] = i * y - _, e[4] = e[12] = e[20] = e[28] = t - s * y - _, e[6] = e[14] = e[22] = e[30] = t - _, e[1] = e[3] = e[5] = e[7] = -x, e[9] = e[11] = e[13] = e[15] = n * y - x, e[17] = e[19] = e[21] = e[23] = r - a * y - x, e[25] = e[27] = e[29] = e[31] = r - x, this.getBuffer("aPosition").update();
    }
    updateUvs() {
      const e = this.uvs;
  ...
  class Ze {
    ...
    _initGPUSprite(e) {
      const t = e._gpuData[this._renderer.uid] = new qt(), r = t;
  ...
  function ir(o, e) {
    ...
    const a = B.shared;
    a.copyFrom(o._tileTransform.matrix), a.tx /= o.width, a.ty /= o.height, a.invert(), a.scale(o.width / r, o.height / i), rr(e, 2, 0, a);
  }
  const L = new er();
  class sr {
    constructor() {
      this.canBatch = true, this.geometry = new oe({
        indices: L.indices.slice(),
  ...
  class et {
    constructor(e) {
      this._state = X.default2d, this._renderer = e;
    }
    validateRenderable(e) {
    ...
    addRenderable(e, t) {
      ...
      } else r.break(t), i.shader || (i.shader = new Zt()), this.updateRenderable(e), t.add(e);
    }
    execute(e) {
      ...
      r.uTransformMatrix = e.groupTransform, r.uRound = this._renderer._roundPixels | e._roundPixels, $(e.groupColorAlpha, r.uColor, 0), this._state.blendMode = ne(e.groupBlendMode, e.texture._source), this._renderer.encoder.draw({
        geometry: L,
        shader: t,
        state: this._state
      });
    ...
    _updateBatchableMesh(e) {
      ...
      i.addressMode !== "repeat" && (i.addressMode = "repeat", i.update()), ir(e, r.uvs), tr(e, r.positions);
    }
    destroy() {
      this._renderer = null;
    }
    _updateCanBatch(e) {
      ...
      let i = true;
      return this._renderer.type === ie.WEBGL && (i = this._renderer.context.supports.nonPowOf2wrapping), t.canBatch = r.textureMatrix.isSimple && (i || r.source.isPowerOfTwo), t.canBatch;
    }
  }
  et.extension = {
    type: [
  ...
  class tt {
    ...
    addRenderable(e, t) {
      ...
      we(e, r), e._didTextUpdate && (e._didTextUpdate = false, this._updateContext(e, r)), this._renderer.renderPipes.graphics.addRenderable(r, t), r.context.customShader && this._updateDistanceField(e);
    }
    updateRenderable(e) {
      const t = this._getGpuBitmapText(e);
    ...
    _updateContext(e, t) {
      const { context: r } = t, i = Ut.getFont(e.text, e._style);
      r.clear(), i.distanceField.type !== "none" && (r.customShader || (r.customShader = new lr(this._renderer.limits.maxBatchableTextures)));
      const s = D.graphemeSegmenter(e.text), n = e._style;
      let a = i.baseLineOffset;
      const u = Ie(s, n, i, true), l = n.padding, d = u.scale;
      ...
      for (let p = 0; p < u.lines.length; p++) {
        const g = u.lines[p];
        for (let b = 0; b < g.charPositions.length; b++) {
          const C = g.chars[b], U = i.chars[C];
          if (U == null ? void 0 : U.texture) {
            const M = U.texture;
            r.texture(M, f || "black", Math.round(g.charPositions[b] + U.xOffset), Math.round(a + U.yOffset + x), M.orig.width, M.orig.height);
          }
        }
        a += _;
      }
  ...
  class rt {
    ...
    async _updateGpuText(e) {
      ...
      s && (s.structureDidChange = true), t.generatingTexture = false, te(t, e);
    }
    _getGpuText(e) {
      return e._gpuData[this._renderer.uid] || this.initGpuText(e);
    }
    initGpuText(e) {
      ...
      return t.renderable = e, t.transform = e.groupTransform, t.texture = w.EMPTY, t.bounds = {
        minX: 0,
        maxX: 1,
        minY: 0,
        maxY: 0
  ...
  function hr() {
    const { userAgent: o } = se.get().getNavigator();
  ...
  function it(o, e, t, r) {
    ...
    const s = S.getOptimalTexture(i.width, i.height, r, false);
  ...
  async function xr(o) {
    const e = o.filter((t) => P.has(`${t}-and-url`)).map((t) => {
      if (!Z.has(t)) {
        ...
        }), Z.set(t, Promise.all(i).then((s) => s.join(`
`)));
      }
      return Z.get(t);
    });
...
`);
  ...
  function yr(o, e) {
    const t = H.getOptimalCanvasAndContext(o.width, o.height, e), { context: r } = t;
  ...
  class st {
    ...
    async _buildTexturePromise(e) {
      const { text: t, style: r, resolution: i, textureStyle: s } = e, n = K.get(He), a = pr(t, r), u = await xr(a), l = It(t, r, u, n), d = Math.ceil(Math.ceil(Math.max(1, l.width) + r.padding * 2) * i), c = Math.ceil(Math.ceil(Math.max(1, l.height) + r.padding * 2) * i), h = n.image, f = 2;
      h.width = (d | 0) + f, h.height = (c | 0) + f;
      const y = _r(t, r, i, u, n);
      await br(h, y, hr() && a.length > 0);
      const _ = h;
      let x;
      this._createCanvas && (x = yr(h, i));
      const p = it(x ? x.canvas : _, h.width - f, h.height - f, i);
      return s && (p.source.style = s), this._createCanvas && (this._renderer.texture.initSource(p.source), H.returnCanvasAndContext(x)), K.return(n), p;
    }
    ...
    _cleanUp(e) {
      S.returnTexture(e, true), e.source.resource = null, e.source.uploadMethodId = "unknown";
    }
    destroy() {
      this._renderer = null;
  ...
  class nt {
    ...
    addRenderable(e, t) {
      ...
      if (e._didTextUpdate) {
        const i = e._autoResolution ? this._renderer.resolution : e.resolution;
        (r.currentKey !== e.styleKey || e.resolution !== i) && this._updateGpuText(e), e._didTextUpdate = false, te(r, e);
      }
      this._renderer.renderPipes.batch.addToBatch(r, t);
    }
    updateRenderable(e) {
  ...
  class at {
    ...
    getTexture(e, t, r, i) {
      typeof e == "string" && (T("8.0.0", "CanvasTextSystem.getTexture: Use object TextOptions instead of separate arguments"), e = {
        text: e,
        style: r,
        resolution: t
      ...
      const { text: s, style: n, textureStyle: a } = e, u = e.resolution ?? this._renderer.resolution, { frame: l, canvasAndContext: d } = j.getCanvasAndContext({
        text: s,
        style: n,
        resolution: u
      }), c = it(d.canvas, l.width, l.height, u);
      if (a && (c.source.style = a), n.trim && (l.pad(n.padding), c.frame.copyFrom(l), c.frame.scale(1 / u), c.updateUvs()), n.filters) {
        ...
        return this.returnTexture(c), j.returnCanvasAndContext(d), h;
      }
      return this._renderer.texture.initSource(c._source), j.returnCanvasAndContext(d), c;
    }
    returnTexture(e) {
      const t = e.source;
      t.resource = null, t.uploadMethodId = "unknown", t.alphaMode = "no-premultiply-alpha", S.returnTexture(e, true);
    }
    renderTextToCanvas() {
